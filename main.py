...
class AdvancedPowerPointGenerator:
    ...
    def parse_markdown(self) -> List[MarkdownSlide]:
        """Парсит Markdown файл в структуру слайдов с объединением разделов и заголовков"""
        ...
        for line in lines:
            ...
            # H1 заголовок - запоминаем как главный раздел (не создаем слайд!)
            if line.startswith('# '):
                # Первый H1 создаем как отдельный титульный слайд
                if not current_h1_section:
                    title = line[2:].strip()
                    slides.append(MarkdownSlide("section", title))
                
                # Запоминаем текущий H1 раздел
                current_h1_section = line[2:].strip()
                current_h2_section = ""  # Сбрасываем H2
            
            # H2 заголовок - запоминаем как подраздел (не создаем слайд!)
            elif line.startswith('## '):
                current_h2_section = line[3:].strip()
            
            # H3 заголовки - создают content слайд с комбинированным заголовком
            elif line.startswith('### '):
                ...
                # Создаем комбинированный заголовок с переносом строки после ":"
                h3_title = line[4:].strip()
                
                if current_h2_section:
                    # Формат: "H2:\nH3" (с переносом строки)
                    combined_title = f"{current_h2_section}:\n{h3_title}"
                elif current_h1_section:
                    # Формат: "H1:\nH3" (если нет H2, с переносом строки)
                    combined_title = f"{current_h1_section}:\n{h3_title}"
                else:
                    # Только H3
                    combined_title = h3_title
                
                current_slide = MarkdownSlide("content", combined_title)
                current_content_lines = []
        ...
        for i, slide in enumerate(slides[:5]):  # Первые 5 слайдов
            print(f"  Слайд {i+1} ({slide.slide_type}): {slide.title[:60]}...")
    ...
    def create_presentation_from_template(self, slides: List[MarkdownSlide]) -> None:
        ...
        try:
            ...
            for slide_data in slides:
                ...
                # Добавляем изображения на ВСЕ слайды
    ...
    def _update_content_slide(self, slide, title: str, content: str):
        ...
        try:
            ...
            if content_shape and content:
                self._set_formatted_content(content_shape, content)
            ...
            if len(text_shapes) >= 2 and content:
                self._set_formatted_content(text_shapes[1], content)
    
    def _format_markdown_content(self, content: str) -> str:
        """Форматирует Markdown контент для PowerPoint (без лишних bullet points)"""
        if not content:
            return ""
        ...
        for line in lines:
            ...
            # Убираем markdown разметку и НЕ добавляем лишние bullet points
            if line.startswith('• '):
                # Убираем символ bullet point - шаблон сам добавит
                ...
                # Markdown bullet points - убираем символы
                clean_line = line[2:].strip()
                formatted_lines.append(clean_line)
            else:
                # Обычный текст - оставляем как есть, БЕЗ добавления bullet points
                formatted_lines.append(line)
        
        return '\n'.join(formatted_lines)
    
    def _set_formatted_content(self, shape, content: str):
        """Устанавливает контент с поддержкой **bold** форматирования"""
        if not content:
            return
        ...
        for line_idx, line in enumerate(lines):
            ...
            # Убираем markdown разметку bullet points
            if line.startswith('• '):
                clean_line = line[2:].strip()
            elif line.startswith('* ') or line.startswith('- '):
                clean_line = line[2:].strip()
            ...
            # Обрабатываем **bold** форматирование
            self._add_formatted_text_to_paragraph(paragraph, clean_line)
        
        # Применяем базовое форматирование
        self._format_content_text(shape)
    
    def _add_formatted_text_to_paragraph(self, paragraph, text: str):
        """Добавляет текст в параграф с поддержкой **bold** разметки"""
        # Находим все **bold** фрагменты
        import re
        
        # Паттерн для поиска **text**
        bold_pattern = r'\*\*(.*?)\*\*'
        
        current_pos = 0
        
        for match in re.finditer(bold_pattern, text):
            # Добавляем обычный текст перед bold
            before_bold = text[current_pos:match.start()]
            if before_bold:
                run = paragraph.add_run()
                run.text = before_bold
            
            # Добавляем bold текст
            bold_text = match.group(1)  # Текст без **
            run = paragraph.add_run()
            run.text = bold_text
            run.font.bold = True
        ...
        # Если не было никаких bold фрагментов, добавляем весь текст как обычный
        if not re.search(bold_pattern, text):
            run = paragraph.add_run()
            run.text = text
